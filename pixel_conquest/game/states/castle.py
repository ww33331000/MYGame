...
class CastleState(BaseState):
    ...
    def render(self, screen):
        ...
        if self.settlement:
            ...
            title_surface = self.font.render(title, True, Settings.COLORS['gray'])
            ...
            # 管辖村庄
            if self.settlement.villages:
                villages_text = f"管辖村庄: {len(self.settlement.villages)}"
                villages_surface = self.small_font.render(villages_text, True, Settings.COLORS['white'])
                screen.blit(villages_surface, (50, 200))
